  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (found) {
        ...
        setRelatedProducts(getRelatedProducts(id, 4) as Product[]);
      }
      setLoading(false);
    }, 200);
              ...
              <div className="grid grid-cols-4 gap-2 sm:gap-3">
                {images.map((img, index) => (
                  <button 
                    ...
                    className={`aspect-square rounded-lg sm:rounded-xl overflow-hidden cursor-pointer border-2 transition-colors ${
                      index === selectedImage 
                        ? 'border-primary' 
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mb-3 sm:mb-4">
              <div className="flex text-secondary">
                {[1,2,3,4,5].map(i => (
                  <span key={i} className="material-symbols-outlined text-sm sm:text-base">star</span>
                ))}
              </div>
              <span className="font-body text-xs sm:text-sm text-text-muted">(42 reviews)</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
              ...
              <button className="hidden sm:flex size-12 sm:size-14 rounded-full border border-secondary/30 items-center justify-center text-text-muted hover:text-primary hover:border-primary transition-colors bg-white dark:bg-slate-800 min-h-[44px] min-w-[44px]">
                <span className="material-symbols-outlined">favorite</span>
              </button>
            </div>

            {/* Features */}
            <div className="border-t border-secondary/20 pt-6 sm:pt-8 flex flex-col gap-3 sm:gap-4">
              <div className="flex items-start gap-3 sm:gap-4">
                <span className="material-symbols-outlined text-secondary mt-0.5 text-lg sm:text-xl">auto_awesome</span>
                <div>
                  <h4 className="font-bold font-body text-sm sm:text-base text-primary dark:text-cream">Handcrafted with Care</h4>
                  <p className="text-xs sm:text-sm text-text-muted font-body">Each piece is uniquely made by skilled artisans.</p>
                </div>
              </div>
              <div className="flex items-start gap-3 sm:gap-4">
                <span className="material-symbols-outlined text-secondary mt-0.5 text-lg sm:text-xl">local_shipping</span>
                <div>
                  <h4 className="font-bold font-body text-sm sm:text-base text-primary dark:text-cream">Nationwide Delivery</h4>
                  <p className="text-xs sm:text-sm text-text-muted font-body">2-4 days within Dhaka, 3-7 days outside.</p>
                </div>
              </div>
              <div className="flex items-start gap-3 sm:gap-4">
                <span className="material-symbols-outlined text-secondary mt-0.5 text-lg sm:text-xl">eco</span>
                <div>
                  <h4 className="font-bold font-body text-sm sm:text-base text-primary dark:text-cream">Premium Materials</h4>
                  <p className="text-xs sm:text-sm text-text-muted font-body">High-quality yarns and eco-friendly packaging.</p>
                </div>
              </div>
            </div>
          </div>
